...
public class StudentCompleteGrades extends AppCompatActivity {
    private ImageButton btnBackLogin;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

        // Remove the following line since you don't want to set the content view of the base activity
        setContentView(R.layout.activity_student_complete_grades);
        ...
        addHeaderTextView(headerRow, "1st\nQuarter", dpToPx(70));  // Adjust width for grading columns
        addHeaderTextView(headerRow, "2nd\nQuarter", dpToPx(70));  // Adjust width for grading columns
        ...
        if (intent != null) {
            ...
            if (subjects != null && firstGrades != null && secondGrades != null
                ...
                Log.d("StudentCompleteGrades", "Invalid data received. Subjects: " + subjects + ", FirstGrades: " + firstGrades + ", SecondGrades: " + secondGrades);
    ...
    // Add this function to the StudentCompleteGradesDisplayActivity

    private void addSubjectRow(TableLayout tableLayout, String subject, ArrayList<String> firstGrades, ArrayList<String> secondGrades) {
        ...
        subjectTextView.setLayoutParams(new TableRow.LayoutParams(
                TableRow.LayoutParams.WRAP_CONTENT,
                TableRow.LayoutParams.WRAP_CONTENT));
        subjectTextView.setPadding(16, 0, 16, 0);

        TextView gradesTextView = new TextView(this);
        String firstGradesString = formatGrades(firstGrades);
        gradesTextView.setText(firstGradesString);
        gradesTextView.setLayoutParams(new TableRow.LayoutParams(
                TableRow.LayoutParams.WRAP_CONTENT,
                TableRow.LayoutParams.WRAP_CONTENT));
        gradesTextView.setPadding(16, 0, 16, 0);

        TextView grades2TextView = new TextView(this);
        String secondGradesString = formatGrades(secondGrades);
        grades2TextView.setText(secondGradesString);
        grades2TextView.setLayoutParams(new TableRow.LayoutParams(
                TableRow.LayoutParams.WRAP_CONTENT,
                TableRow.LayoutParams.WRAP_CONTENT));
        grades2TextView.setPadding(16, 0, 16, 0);

        TextView finalGradesTextView = new TextView(this);
        double average = calculateAverage(firstGrades, secondGrades);
        String averageGrade = String.format("%.2f", average); // Format average to two decimal places
        finalGradesTextView.setText(averageGrade);
        finalGradesTextView.setLayoutParams(new TableRow.LayoutParams(
                TableRow.LayoutParams.WRAP_CONTENT,
                TableRow.LayoutParams.WRAP_CONTENT));
        finalGradesTextView.setPadding(16, 0, 16, 0);

        setGradingStyle(subjectTextView);
        setGradingStyle(gradesTextView);
        setGradingStyle(grades2TextView);
        setGradingStyle(finalGradesTextView);

        tableRow.addView(subjectTextView);
        tableRow.addView(gradesTextView);
        tableRow.addView(grades2TextView);
        tableRow.addView(finalGradesTextView);
        tableLayout.addView(tableRow);

        // Calculate GWA and update TextView
        TextView gwaTextView = findViewById(R.id.gwa);
        double currentGwa = Double.parseDouble(gwaTextView.getText().toString());
        int count = tableLayout.getChildCount(); // Get the total number of rows including the header
        currentGwa = (currentGwa * (count - 1) + average) / count; // Update GWA with the new average
        gwaTextView.setText(currentGwa + "GWA"); // Format and set the updated GWA

        // Determine the ranking based on GWA
        TextView rankingTextView = findViewById(R.id.ranking);
        if (currentGwa < 90) {
            rankingTextView.setText("No Honor");
        } else if (currentGwa >= 90 && currentGwa <= 94) {
            rankingTextView.setText("With Honor");
        } else if (currentGwa >= 95 && currentGwa <= 97) {
            rankingTextView.setText("With High Honor");
        } else if (currentGwa >= 98 && currentGwa <= 100) {
            rankingTextView.setText("With Highest Honor");
        }
    }

    // Method to calculate the average of firstGrades and secondGrades
    private double calculateAverage(ArrayList<String> firstGrades, ArrayList<String> secondGrades) {
        double total = 0;
        int count = 0;
        for (String grade : firstGrades) {
            if (grade != null) {
                total += Double.parseDouble(grade);
                count++;
            }
        }
        for (String grade : secondGrades) {
            if (grade != null) {
                total += Double.parseDouble(grade);
                count++;
            }
        }
        return count > 0 ? total / count : 0; // Return the average or 0 if count is 0 to avoid division by zero
    }

    // Method to format grades ArrayList as a string

    private String formatGrades(ArrayList<String> grades) {
        StringBuilder stringBuilder = new StringBuilder();
        for (String grade : grades) {
            if (grade != null) {
                stringBuilder.append(grade);
                stringBuilder.append("\n"); // Add a new line after each grade
            }
        }
